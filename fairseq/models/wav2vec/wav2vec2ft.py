...
class Wav2Vec2FTModel(BaseFairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--attention-type",
            type=str,
            help="attention type to be used",
        )

        parser.add_argument(
            "--feature-maps",
            type=str,
            choices=['FAVOR', 'default'],
            help="Feature map to be used",
        )
        parser.add_argument(
            "--expand-dims",
            type=int,
            help="Number of dimensions to expand to",
        )

        parser.add_argument(
            "--clusters",
            type=int,
            help="number of clusters (applicable to clustered attention)",
        )

        parser.add_argument(
            "--topk",
            type=int,
            help="number of topk keys (applicable to improved-clustered attention)",
        )

        parser.add_argument(
            "--bits",
            type=int,
            help="number of bits (applicable to clustered attention)",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            choices=["True", "False"],
            help="use hash bias for hashing (applicable to clustered attention)",
        )

        parser.add_argument(
            "--length_limit",
            type=int,
            help="length limit for conditional-attention",
        )
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        feat_map = args.feature_maps
        if args.feature_maps == 'FAVOR':
            feature_map_func = partial(Favor, n_dims=expand_dims)
        else:
        ...
        builder = AttentionBuilder.from_kwargs(
            ...
            topk=args.topk,
            bits=args.bits,
            hash_bias=strtobool(args.hash_bias),
    ...
    def extract_features(self, x, padding_mask=None):
        ...
        length_mask = None
        if padding_mask:
            input_lengths = 1 - padding_mask.sum(-1)
...
@register_model_architecture("wav2vec2ft", "wav2vec2ft")
def base_architecture(args):
    ...
    args.attention_type = getattr(args, "attention_type", "full")
    args.feature_maps = getattr(args, "feature_maps", "default")
    args.expand_dims = getattr(args, "expand_dims", 256)
    args.clusters = getattr(args, "clusters", 200)
    args.topk = getattr(args, "topk", 32)
    args.bits = getattr(args, "bits", 63)
    args.hash_bias = getattr(args, "hash_bias", "True")
    args.length_limit = getattr(args, "length_limit", 512)
